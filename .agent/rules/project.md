...
- RLS(행 수준 보안)는 Supabase CLI 마이그레이션으로 관리합니다. 정책 변경은 마이그레이션 파일로 작성해 버전 관리하세요.
- `posts` 테이블의 권한은 `posts.user_id`(또는 기존 `author_id`)와 `auth.uid()`를 비교하여 설정합니다.
- 콘솔에서 임시 테스트는 가능하지만, 영구 적용은 마이그레이션으로 남길 것(콘솔 직접 편집 금지 권장).
